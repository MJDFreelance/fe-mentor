...
import {Favourite} from "@/app/entertainment-app/components/Favourites";

const Page = () => {
    const {results:data} = useSearch();

    return (
        <>
            <section className={`flex flex-col gap-6 text-white`}>
                <h2 className={`font-light text-[20px] -tracking-[.31px]`}>Trending</h2>
                <div className={`scrollbar-custom flex gap-4 overflow-auto snap-x snap-mandatory`}>
                    {data.filter(item => item.isTrending).map((item, index) => (
                        <div key={`item-${index}`}
                             className={`snap-start flex items-end relative p-4 rounded-[8px] h-[140px] w-[240px] md:h-[230px] md:w-[470px] flex-shrink-0 overflow-hidden`}>
                            <Image className={`z-0`} fill
                                   src={item.thumbnail.trending?.small ?? '/entertainment-app/icon-nav-home.svg'}
                                   alt={item.title}/>
                            <Favourite id={item.id??''} className={`absolute top-0 right-0`}  />
                            <div
